...
exports.createBootcamp = asyncHandler(async (req, res, next) => {
    ...
    // if the user is not an admin, they can only dd one bootcamp
...
exports.updateBootcamp = asyncHandler(async (req, res, next) => {
    ...
    if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(
            new ErrorResponse(`The user with ID ${req.user.id}  is not authorized to update this bootcamp`, 401)
        );
    }
...
exports.deleteBootcamp = asyncHandler(async (req, res, next) => {
    ...
    if (bootcamp.user.toString() !== req.user.id && req.user.role !== 'admin') {
        return next(
            new ErrorResponse(`The user with ID ${req.user.id}  is not authorized to delete this bootcamp`, 401)
        );
    }
